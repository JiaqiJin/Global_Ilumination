...
void Model::InitModel(ID3D12Device* device, CommandContext* cmdObj)
{
    switch (modelType)
    {
    case DEFAULT_MODEL:
    ...
    CreateBuffer(device);
    CreateUploadBuffer(device, cmdObj);
}
...
void Model::CreateUploadBuffer(ID3D12Device* device, CommandContext* cmdObj)
{
    ThrowIfFailed(device->CreateCommittedResource(
