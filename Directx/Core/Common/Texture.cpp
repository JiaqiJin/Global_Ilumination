...
bool Texture::UploadTexture(ID3D12Device* device, CommandContext* cmdContext) 
{
	...
	UpdateSubresources(cmdContext->getCommandListPtr(), textureBuffer.Get(), UploadHeap.Get(), 0, 0, 1, &textureData);

	cmdContext->getCommandListPtr()->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(textureBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
...
bool Texture::initializeTextureBuffer(ID3D12Device* device, CommandContext* cmdContext) 
{
	...
	if (!(UploadTexture(device, cmdContext)))
	{
...
RenderTexture::RenderTexture(UINT _width, UINT _height, DXGI_FORMAT _format) 
	: Texture(), width(_width), height(_height) 
{
	textureDESC = {};
...
bool RenderTexture::UploadTexture(ID3D12Device* device, CommandContext* cmdContext)
{
	return true;
}

bool RenderTexture::initializeTextureBuffer(ID3D12Device* device, CommandContext* cmdContext) 
{
	if (!(CreateTextureBuffer(device))) 
	{
